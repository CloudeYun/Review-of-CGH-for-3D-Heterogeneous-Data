...
# -*- coding: utf-8 -*-
"""
Perspective Light Field Rendering with Parallel Optical Axes

Author: Hao Yun
Date: 2026-03-23

Description:
    This script renders a perspective light field from a colored triangle mesh
    using Open3D ray casting. The mesh is centered and uniformly scaled so that
    its thickness along the viewing direction matches a target physical depth range.

    Unlike standard converging multi-view rendering, this implementation uses
    parallel optical axes: each virtual camera is translated across the view grid,
    while the optical axis direction remains fixed.

Main features:
    - Perspective multi-view rendering
    - Parallel optical axis camera model
    - Material-based triangle colors
    - Double-sided mesh expansion
    - Shaded RGB rendering
    - Export of images, poses, intrinsics, and timing logs

Example:
    python ./dataset/Obj2LF_pers.py \
        --input_obj ./dataset/BunnyDragon.obj \
        --output_dir ./dataset/LightField/LF_pers_40x40_800 \
        --img_width 800 \
        --img_height 800 \
        --fov_deg 7 \
        --nu 40 \
